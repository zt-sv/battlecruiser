 ...
 *
 * @throw   {Error}     throw Error when object is invalid
 */
PropChecker.validate = function(obj, config) {
    for (let key in config) {
        ...
        if (config[key] instanceof PropChecker) {
            config[key].check(key, obj[key]);
        } else if (h.isArray(config[key])) {
            config[key].forEach(checker => {
                if (checker instanceof PropChecker) {
                    checker.check(key, obj[key]);
                }
            });
        } else if (h.isObject(config[key])) {
            PropChecker.validate(obj[key], config[key]);
        }
    }
};
